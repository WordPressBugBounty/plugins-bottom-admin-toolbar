...
if ( ! class_exists( 'BAB_Locahl_Banner' ) ) {
	/**
	 * Branded Locahl marketing banner for wp-admin.
	 */
	class BAB_Locahl_Banner {
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_bab_locahl_dismiss_banner', array( $this, 'dismiss_banner' ) );
		}
		...
		private function get_meta_key() {
			return 'locahl_banner_dismissed_bottom-admin-toolbar';
		}
		...
		public function enqueue_scripts( $hook ) {
			...
			wp_enqueue_style(
				'bab-locahl-banner',
				BAB_ASSETS_URL . 'locahl-banner.css',
				array( 'bab-locahl-banner-fonts' ),
				self::ASSET_VERSION
			);

			wp_enqueue_script(
				'bab-locahl-banner',
				BAB_ASSETS_URL . 'locahl-banner.js',
				array( 'jquery' ),
				self::ASSET_VERSION,
				true
			);
			...
			wp_localize_script(
				...
				array(
					...
					'action'   => 'bab_locahl_dismiss_banner',
					'nonce'    => wp_create_nonce( 'bab_locahl_dismiss_banner' ),
				)
			);
		}
		...
		public function render_banner() {
			...
			?>
			<div class="notice locahl-promo-banner bab-locahl-banner">
		...
		public function dismiss_banner() {
			...
			check_ajax_referer( 'bab_locahl_dismiss_banner', 'nonce' );
			update_user_meta( get_current_user_id(), $this->get_meta_key(), '1' );
			wp_send_json_success();
		}
	}

	new BAB_Locahl_Banner();
}
